...
local state = {
    ...
    -- 2026-08-14: defaults to OFF -- this was silently logging a burst every
    -- single time the player aimed, in every normal play session, whether or
    -- not the spine/aim-alignment investigation this probe supports was
    -- actually active. Kept in the mod (not deleted) since the underlying
    -- "gun points left after spine straightening" issue is still unresolved
    -- (see re2_vr_torso_twist_status memory) -- flip back on only when
    -- actively diagnosing that.
...
re.on_draw_ui(function()
    ...
    if not imgui.tree_node("Aim alignment probe (diagnostic, off by default)") then return end

    imgui.text("Read-only, logs to re2_framework_log.txt")
    imgui.text("Prefix to grep for: [aim_align_probe]")
    imgui.text_colored(
        "For the unresolved 'gun points left after spine straightening' issue --",
        0xFF88CCFF)
    imgui.text_colored(
        "leave auto-capture OFF unless you're actively diagnosing that.",
        0xFF88CCFF)
