...
function slide.get_dock_state()
    ...
    if cos_blend > 0.001 then
        return cos_blend,
            rawget(_G, "__vr_cosmetic_dock_hand_world_pos"),
            rawget(_G, "__vr_cosmetic_dock_hand_world_rot"),
            false
    end
    return real_blend,
        rawget(_G, "__vr_slide_hand_world_pos"),
...
local function resolve_slide_dock_hand_target(slide_pos, blend, fallback_pos)
    ...
    -- 2026-08-14: found live -- __vr_slide_dock_blend_from_pos is NEVER SET
    -- (capture_slide_dock_blend_origin, above, is dead code, unreferenced
    -- anywhere in the mod) -- so this always falls through to __vr_lh_world,
    -- meaning "from_pos" is a LIVE, every-frame-changing read, not a fixed
    -- blend origin. That's normally fine (an exponential ease, __vr_lh_world
    -- mostly mirrors this same function's own previous output via
    -- __vr_lh_joint_pos while docked) -- but __vr_lh_slide_ik_override
    -- flickers false for isolated frames even with the near/grip debounce
    -- fixes above (confirmed live via slide_rack_dist_probe logging), and on
    -- those frames __vr_lh_world falls back to publish_vr_hand_globals's OWN
    -- camera-relative reconstruction. Reading that tainted value here as
    -- "from_pos" kicks this blend's trajectory away from the dock for a
    -- moment, then the exponential ease pulls it back -- a visible snap-and-
    -- recover, worse the more the head has turned. Dropped the explicit
    -- get_fp_style_hand_world_pos fallback entirely -- go straight to the
    -- raw controller position (already proven reliable elsewhere in this
    -- mod) instead of ever deliberately reaching for the camera-relative
    -- reconstruction in this specific chain. Properly wiring up
    -- capture_slide_dock_blend_origin for a real fixed-origin blend would be
    -- a more thorough fix -- left as a known follow-up, not attempted here.
    -- __vr_lh_world itself is only guaranteed to be the stable joint while
    -- the override gate is true (publish_vr_hand_globals falls back to the
    -- SAME camera-relative reconstruction internally whenever it isn't) --
    -- checking the gate directly here, not just whether the global happens
    -- to be populated, since a tainted value still reads as "valid".
    local from_pos = rawget(_G, "__vr_slide_dock_blend_from_pos")
    if not vec3_valid(from_pos) and rawget(_G, "__vr_lh_slide_ik_override") == true then
...
local function apply_slide_dock_to_left_arm(priority)
    ...
    -- 2026-08-14: tried tightening this to `<=` (same-priority repeats
    -- within a frame also skipped, not just strictly-lower ones) to stop
    -- on_post_update_ik's escalating-priority repeat calls from re-running
    -- the full custom bend-IK solve multiple times per frame (suspected of
    -- causing a residual, less-severe version of the hand-loop bug at
    -- certain hand positions -- see re2_vr_slide_rack_loop_regression_
    -- 2026_08_14 memory). CONFIRMED LIVE this broke something else: the
    -- visual hand-reach-onto-the-slide animation stopped appearing
    -- entirely (grip+trigger racking still worked functionally, just no
    -- visible hand travel, and the player had to already be close to the
    -- weapon for it to engage at all). Root cause of THAT not chased down.
    -- Reverted to the original `<` -- back to only blocking strictly-lower-
    -- priority repeats, restoring the visual animation (player confirmed:
    -- "felt better than before"). The escalating-priority repeat-call
    -- pattern this leaves in place is a known, deliberately deferred
    -- follow-up, not the cause chased down and fixed here.
...
local function on_pre_update_ik(args)
    ...
    -- 2026-08-14: found live -- player reproduced a rapid two-state hand
    -- loop (real hand stationary, raw controller distance flat, but
    -- __vr_lh_joint_pos alternated between two fixed values nearly every
    -- frame) at a specific hand position relative to the weapon. Root
    -- cause: this branch and apply_slide_dock_to_left_arm (above) are TWO
    -- INDEPENDENT writers of the same __vr_lh_joint_pos/arm pose -- this one
    -- writes the native IkArmFit target matrix directly, the other runs a
    -- full custom bend-IK solve (elbow pole hints, clavicle stretch) and
    -- writes joint rotations directly, bypassing the native solver
    -- entirely. Both are gated on essentially the same slide_blend>0
    -- condition and can both fire the same frame from different hooks
    -- (on_pre_update_ik here is a native IkArmFit.updateIk pre-hook;
    -- apply_slide_dock_to_left_arm is called from re2_vr_reload_ext_2.lua's
    -- own separate hook sites) -- whichever happens to write last that
    -- frame wins, and if that alternates frame to frame (plausible at a
    -- hand pose where the two solvers' outputs diverge most), that alone
    -- reproduces exactly the observed two-state flicker. The existing
    -- slide.dock_apply_frame/dock_apply_prio guard (see
    -- apply_slide_dock_to_left_arm above) already exists for coordinating
    -- exactly this class of same-frame collision -- extending it here:
    -- skip this branch's write whenever the custom bend-IK solve has
    -- already claimed this exact frame, so it stays the sole owner rather
    -- than racing.
    if side == "left" and slide_blend > 0.001 and vec3_valid(slide_pos)
        and rawget(_G, "__vr_pump_fp_passthrough") ~= true
        and slide.dock_apply_frame ~= get_frame_id() then
...
local function on_post_update_ik(retval)
    ...
    if blend > 0.001 and vec3_valid(slide_pos) then
        ...
        if arm_blend > 0.001 then
            ...
            -- 2026-08-14: tried capping this at a fixed 50 (was
            -- 50 + slide.post_ik_count) so repeat same-frame calls would
            -- share a priority tier and get deduped by the guard above --
            -- reverted alongside that guard change, see its comment. Back
            -- to the escalating priority.
