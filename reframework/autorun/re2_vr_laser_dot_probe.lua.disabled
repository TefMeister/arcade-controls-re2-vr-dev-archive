...
local state = {
    ...
    -- 2026-08-15, resumed once more: player's own valid concern -- clicking
    -- an overlay button while physically holding RG in VR means taking a
    -- hand off the controller, which risks dropping/disturbing the aim
    -- state being measured (same lesson already documented in
    -- re2_vr_aim_alignment_probe.lua's header). Auto-arm instead: check
    -- this box BEFORE aiming, then aim normally -- the dump fires itself
    -- the instant IsHold flips true, no button press needed mid-aim.
    -- Auto-clears after firing once so it doesn't spam the log on every
    -- subsequent re-aim.
    auto_dump_hierarchy_on_aim = false,
    hierarchy_was_aiming = false,
    ...
    -- 2026-08-15, resumed once more: app.ropeway.LaserSightController found
    -- (new lead) with a real setPosition method plus a <PointerEffect>
    -- field/getter that's very plausibly the dot itself. Hooking
    -- setPosition, gated to the same burst window as everything else, to
    -- avoid spam.
    laser_sight_ctrl_hook_installed = false,
    pending_ls_ctrl_this = nil,
...
-- 2026-08-15, resumed once more: LaserSightController dump revealed
-- SightEmitJointName = vfx_muzzle3 -- the sight's own TRUE emission joint,
-- different from vfx_muzzle1/vfx_muzzle2 (the only names
-- get_muzzle_ground_truth ever checked). setPosition confirmed to never
-- fire, consistent with the dot just being ordinary native skeletal
-- parenting to this joint -- if THIS joint behaves differently than the one
-- measured all session, that could be the real answer. Reads it directly by
-- name off the WEAPON's own transform (same convention as
-- SightEmitJointName being a weapon-local joint).
local function get_vfx_muzzle3_ground_truth()
    ...
    if not weapon_tf then return nil, nil end
    local joint = safe(function() return weapon_tf:call("getJointByName", "vfx_muzzle3") end)
    if not joint then return nil, nil end
    local pos = safe(function() return joint:call("get_Position") end)
    local fwd = safe(function() return joint:call("get_AxisZ") end)
    return pos, fwd
end

-- 2026-08-15, resumed once more: CreatedEffectContainer (PointerEffect)
-- exposes get_effectPositions() directly -- a real accessor for the
-- effect's own live position(s), rather than hooking a write event (three
-- previous hooks all confirmed dead: WeaponArm setter, Stamp.set_Color,
-- LaserSightController.setPosition). POLLING this every logged frame
-- sidesteps needing to catch a one-time/rare write entirely. Return shape
-- is unconfirmed -- tries treating the result as a list (get_elements(),
-- same sugar-method lesson as the hierarchy dump's component listing) and
-- falls back to treating it as a single vec3-like value directly.
local function get_pointer_effect_world_pos()
    if not re2.weapon then return nil end
    local weapon_go = safe(function() return re2.weapon:call("get_GameObject") end)
    local weapon_tf = weapon_go and safe(function() return weapon_go:call("get_Transform") end)
    if not weapon_tf then return nil end
    local child = safe(function() return weapon_tf:call("get_Child") end)
    local ls_go = nil
    while child do
        local go = safe(function() return child:call("get_GameObject") end)
        local name = go and safe(function() return go:call("get_Name") end)
        if name == "LaserSight" then ls_go = go break end
        child = safe(function() return child:call("get_Next") end)
    end
    if not ls_go then return nil end

    local ctrl = safe(function() return ls_go:call("getComponent(System.Type)", sdk.typeof("app.ropeway.LaserSightController")) end)
    local container = ctrl and safe(function() return ctrl:call("get_PointerEffect") end)
    if not container then return nil end

    local iter = safe(function() return container:call("get_effectPositions") end)
    if not iter then return nil end

    -- 2026-08-15, resumed once more: confirmed via dump_effect_positions_probe()
    -- that get_effectPositions() returns a compiler-generated C# iterator
    -- state machine (type name <get_effectPositions>d__12), NOT a list/array
    -- -- both earlier guessed shapes were wrong for exactly this reason
    -- (get_elements/get_Length/get_Count all genuinely don't apply to a raw
    -- enumerator). Standard C# IEnumerator consumption: call MoveNext(),
    -- then read Current if it returned true.
    local ok_next, has_next = pcall(function() return iter:call("MoveNext") end)
    if ok_next and has_next then
        local cur = safe(function() return iter:call("get_Current") end)
        return cur
    end
    return nil
end

-- 2026-08-15, resumed once more: pe_pos came back nil for an entire
-- capture (7195/7195 samples) -- neither guessed shape matched. Rather than
-- guess a third shape, this dumps exactly what get_effectPositions()
-- actually returns: whether the call itself returns nil, its live type
-- name, whether get_elements()/get_Length()-style array access works, and
-- a raw tostring() as a fallback -- real information either way, one-shot.
local function dump_effect_positions_probe()
    local lines = {}
    if not re2.weapon then
        state.last_dump = "no re2.weapon (equip/aim a weapon first)"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end
    local weapon_go = safe(function() return re2.weapon:call("get_GameObject") end)
    local weapon_tf = weapon_go and safe(function() return weapon_go:call("get_Transform") end)
    local child = weapon_tf and safe(function() return weapon_tf:call("get_Child") end)
    local ls_go = nil
    while child do
        local go = safe(function() return child:call("get_GameObject") end)
        local name = go and safe(function() return go:call("get_Name") end)
    ...
    if not ls_go then
        state.last_dump = "LaserSight GameObject not found"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end
    ...
    local ctrl = safe(function() return ls_go:call("getComponent(System.Type)", sdk.typeof("app.ropeway.LaserSightController")) end)
    local container = ctrl and safe(function() return ctrl:call("get_PointerEffect") end)
    table.insert(lines, "container (PointerEffect) found: " .. tostring(container ~= nil))
    if not container then
        state.last_dump = table.concat(lines, "\n")
        log.info("[laser_dot_probe] effectPositions probe:\n" .. state.last_dump)
        return
    end

    local ok_call, positions = pcall(function() return container:call("get_effectPositions") end)
    table.insert(lines, string.format("get_effectPositions() call ok=%s  returned nil=%s",
        tostring(ok_call), tostring(positions == nil)))

    if positions then
        table.insert(lines, "lua type(positions) = " .. type(positions))
        local ptdef = safe(function() return positions:get_type_definition() end)
        local ptname = ptdef and safe(function() return ptdef:get_full_name() end)
        table.insert(lines, "positions live type: " .. tostring(ptname))

        local ok_elem, elements = pcall(function() return positions:get_elements() end)
        table.insert(lines, string.format("get_elements() ok=%s  count=%s",
            tostring(ok_elem), tostring(ok_elem and elements and #elements or "n/a")))
        if ok_elem and elements and #elements > 0 then
            table.insert(lines, "elements[1] = " .. tostring(elements[1]))
        end

        local ok_len, len = pcall(function() return positions:call("get_Length") end)
        table.insert(lines, string.format("get_Length() ok=%s  value=%s", tostring(ok_len), tostring(len)))

        local ok_count, cnt = pcall(function() return positions:call("get_Count") end)
        table.insert(lines, string.format("get_Count() ok=%s  value=%s", tostring(ok_count), tostring(cnt)))

        table.insert(lines, "raw tostring(positions) = " .. tostring(positions))
    end

    -- Also try getPositionForEPV directly (index 0), a plausible single-item
    -- accessor seen in the same dump alongside get_effectPositions.
    local ok_epv, epv_pos = pcall(function() return container:call("getPositionForEPV", 0) end)
    table.insert(lines, string.format("getPositionForEPV(0) ok=%s  value=%s", tostring(ok_epv), tostring(epv_pos)))

    -- 2026-08-15: positions confirmed to be a C# iterator state machine
    -- (<get_effectPositions>d__12), not a list -- verify the standard
    -- MoveNext()/get_Current() consumption pattern actually works, walking
    -- ALL entries this time (get_maxEffectPositionNum implied there could be
    -- more than one), not just the first.
    if positions then
        local count = 0
        while count < 16 do
            local ok_mn, has_next = pcall(function() return positions:call("MoveNext") end)
            if not ok_mn or not has_next then
                table.insert(lines, string.format("MoveNext() #%d: ok=%s has_next=%s -- stopping",
                    count, tostring(ok_mn), tostring(has_next)))
                break
            end
            local cur = safe(function() return positions:call("get_Current") end)
            table.insert(lines, string.format("  [%d] Current = %s", count, tostring(cur)))
            if cur then
                local cx = safe(function() return cur.x end)
                if cx then
                    table.insert(lines, string.format("      as vec3-like: (%s, %s, %s)",
                        tostring(cur.x), tostring(cur.y), tostring(cur.z)))
                end
            end
            count = count + 1
        end
...
-- Item 5 (2026-08-15, resumed once more): app.ropeway.LaserSightController --
-- a genuinely NEW lead, found on the "LaserSight" child GameObject via the
-- full-hierarchy dump (never found by the original 17-mechanism
-- investigation's whole-game type-database name search, nor by anything
-- tried today). Dumps ALL fields (with live values where readable) and ALL
-- methods, walking the full type hierarchy, unfiltered by keyword -- this
-- is a small, specific, dedicated controller class, not the whole engine,
-- so no filtering is needed or wanted (a keyword filter is exactly what
-- could hide the real answer, same lesson as the hierarchy dump itself).
local function find_laser_sight_gameobject()
    if not re2.weapon then return nil end
    local weapon_go = safe(function() return re2.weapon:call("get_GameObject") end)
    local weapon_tf = weapon_go and safe(function() return weapon_go:call("get_Transform") end)
    if not weapon_tf then return nil end
    ...
    while child do
        local go = safe(function() return child:call("get_GameObject") end)
        local name = go and safe(function() return go:call("get_Name") end)
        if name == "LaserSight" then return go end
        child = safe(function() return child:call("get_Next") end)
    end
    return nil
end

local function dump_laser_sight_controller()
    local lines = {}
    local ls_go = find_laser_sight_gameobject()
    if not ls_go then
        state.last_dump = "LaserSight GameObject not found (equip/aim a sight/laser weapon first)"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end

    local ctrl_type = sdk.find_type_definition("app.ropeway.LaserSightController")
    if not ctrl_type then
        state.last_dump = "app.ropeway.LaserSightController: type not found in this game's TDB"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end

    local ctrl = safe(function() return ls_go:call("getComponent(System.Type)", sdk.typeof("app.ropeway.LaserSightController")) end)
    table.insert(lines, "app.ropeway.LaserSightController -- live instance found: " .. tostring(ctrl ~= nil))

    local tdef = ctrl_type
    local depth = 0
    while tdef and depth < 6 do
        local tname = safe(function() return tdef:get_full_name() end) or "?"
        table.insert(lines, string.format("== L%d: %s ==", depth, tname))
        ...
        for _, f in ipairs(fields) do
            ...
            if ctrl then
                if is_static then
                    ...
                    ok_v, value = pcall(function() return f:get_data(ctrl) end)
                end
            end
            table.insert(lines, string.format("  [field] %s = %s", fname or "?",
                (ctrl and ok_v) and tostring(value) or "(no live instance / unreadable)"))
        end
    ...
    -- setPosition confirmed NEVER fires (2026-08-15, live hook, full 36s+
    -- capture window) despite the hook being confirmed installed -- very
    -- likely a one-time setup call (attach to a joint), not a per-frame
    -- recompute. If so, the dot's real position is just ordinary native
    -- skeletal parenting -- walk PointerEffect/Light/Line's actual parent
    -- chain to see what they're really attached to, rather than guessing.
    -- 2026-08-15, resumed once more: first version assumed a fixed
    -- obj -> GameObject -> Transform shape and got "no transform found" for
    -- everything -- the getters almost certainly return something else
    -- (e.g. PointerEffect may be a via.effect.EffectPlayer component
    -- directly, not wrapped in a GameObject). Logging the RAW RETURNED TYPE
    -- first (real information either way), then trying several plausible
    -- shapes in turn instead of assuming just one.
    local function dump_parent_chain(getter_name)
        local obj = safe(function() return ctrl:call(getter_name) end)
        if not obj then
            table.insert(lines, "  " .. getter_name .. ": returned nil")
            return
        end
        local obj_tdef = safe(function() return obj:get_type_definition() end)
        local obj_tname = obj_tdef and safe(function() return obj_tdef:get_full_name() end)
        table.insert(lines, "  " .. getter_name .. " returned type: " .. tostring(obj_tname))

        local tf = nil
        -- shape 1: obj is a Component/Transform-owner -- has get_GameObject()
        local go = safe(function() return obj:call("get_GameObject") end)
        if go then
            tf = safe(function() return go:call("get_Transform") end)
        end
        -- shape 2: obj IS a via.GameObject directly -- has get_Transform()
        if not tf then
            tf = safe(function() return obj:call("get_Transform") end)
        end
        -- shape 3: obj IS a via.Transform already -- has get_Position()
        if not tf then
            local has_pos = safe(function() return obj:call("get_Position") end)
            if has_pos then tf = obj end
        end
        if not tf then
            table.insert(lines, "  " .. getter_name .. ": no transform found (type above, none of the 3 tried shapes matched)")
            return
        end
        local chain = {}
        local cur = tf
        local depth2 = 0
        while cur and depth2 < 8 do
            local cgo = safe(function() return cur:call("get_GameObject") end)
            local cname = cgo and safe(function() return cgo:call("get_Name") end)
            table.insert(chain, cname or "?")
            cur = safe(function() return cur:call("get_Parent") end)
            depth2 = depth2 + 1
        end
        table.insert(lines, "  " .. getter_name .. " parent chain: " .. table.concat(chain, " <- "))
    end

    if ctrl then
        table.insert(lines, "== Parent chains (what each part is actually attached to) ==")
        dump_parent_chain("get_PointerEffect")
        dump_parent_chain("get_Light")
        dump_parent_chain("get_Line")
    end

    state.last_dump = table.concat(lines, "\n")
    log.info("[laser_dot_probe] LaserSightController dump:\n" .. state.last_dump)
end

-- 2026-08-15, resumed once more: PointerEffect confirmed to be a
-- via.effect.script.CreatedEffectContainer -- NOT a GameObject/Transform at
-- all, unlike Light/Line (confirmed ordinary skeletal parenting,
-- Light/Line <- LaserSight <- wp0200 <- pl0000, matching the stable gun
-- mesh). A wrapper for a dynamically-created VFX effect is very plausibly
-- positioned by the effect system's own separate logic, not the transform
-- hierarchy -- dumps its full field+method surface, unfiltered, to find the
-- real accessor.
local function dump_created_effect_container()
    local lines = {}
    local ls_go = find_laser_sight_gameobject()
    if not ls_go then
        state.last_dump = "LaserSight GameObject not found (equip/aim a sight/laser weapon first)"
    ...
    local ctrl = safe(function() return ls_go:call("getComponent(System.Type)", sdk.typeof("app.ropeway.LaserSightController")) end)
    if not ctrl then
        state.last_dump = "LaserSightController instance not found"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end
    local pointer_effect = safe(function() return ctrl:call("get_PointerEffect") end)
    if not pointer_effect then
        state.last_dump = "PointerEffect returned nil"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end

    local tdef = safe(function() return pointer_effect:get_type_definition() end)
    if not tdef then
        state.last_dump = "PointerEffect: could not get type definition"
        log.info("[laser_dot_probe] " .. state.last_dump)
        return
    end

    table.insert(lines, "PointerEffect live type: " .. tostring(safe(function() return tdef:get_full_name() end)))
    ...
    while tdef and depth < 6 do
        ...
        for _, f in ipairs(fields) do
            ...
            if is_static then
                ...
                ok_v, value = pcall(function() return f:get_data(pointer_effect) end)
            end
            table.insert(lines, string.format("  [field] %s = %s", fname or "?",
    ...
    state.last_dump = table.concat(lines, "\n")
    log.info("[laser_dot_probe] CreatedEffectContainer (PointerEffect) dump:\n" .. state.last_dump)
end
...
local function dump_transform_tree(xform, depth, lines, node_count)
    ...
    if go then
        -- 2026-08-15: get_elements() is a REFramework Lua-binding sugar
        -- method on the list wrapper, not a real reflected .NET method --
        -- must be called via direct method syntax (list:get_elements()),
        -- not list:call("get_elements") (which looks up an actual reflected
        -- method by that literal name and silently fails/returns nil since
        -- one doesn't exist). Matches utility/GameObject.lua's own proven
        -- usage exactly (get_components()) -- first version of this dump
        -- used :call() here and silently produced zero component lines for
        -- every single node in the tree.
...
-- Reads PointerEffect/Light/Line's actual world-space Transform position via
-- normal SDK calls on the captured controller instance -- NOT raw argument
-- decoding (same discipline as the Stamp hook above: safer than guessing at
-- how to parse a value-type via.vec3 argument out of raw hook args when a
-- normal call on the object works instead).
local function read_laser_sight_state(ctrl)
    if not ctrl then return "no controller" end
    local function pos_of(getter)
        local obj = safe(function() return ctrl:call(getter) end)
        local go = obj and safe(function() return obj:call("get_GameObject") end)
    ...
    return string.format("PointerEffect=%s  Light=%s  Line=%s",
        pos_of("get_PointerEffect"), pos_of("get_Light"), pos_of("get_Line"))
end

local function try_install_laser_sight_ctrl_hook()
    if state.laser_sight_ctrl_hook_installed then return end
    local ls_go = find_laser_sight_gameobject()
    if not ls_go then return end
    local ctrl_type = sdk.find_type_definition("app.ropeway.LaserSightController")
    if not ctrl_type then return end
    local method = safe(function() return ctrl_type:get_method("setPosition") end)
    if not method then return end

    sdk.hook(method,
        function(args)
            local this_obj = sdk.to_managed_object(args[1])
            state.pending_ls_ctrl_this = this_obj
            if state.burst_remaining <= 0 and not (state.manual_capture_until and os.clock() < state.manual_capture_until) then return end
            log.info(string.format("[laser_dot_probe] LaserSightController.setPosition CALLED frame=%d  before: %s",
                state.frame, read_laser_sight_state(this_obj)))
        end,
        function(retval)
            if state.pending_ls_ctrl_this then
                if state.burst_remaining > 0 or (state.manual_capture_until and os.clock() < state.manual_capture_until) then
                    log.info(string.format("[laser_dot_probe] LaserSightController.setPosition RETURNED frame=%d  after: %s",
                        state.frame, read_laser_sight_state(state.pending_ls_ctrl_this)))
                end
            end
            return retval
        end)
    state.laser_sight_ctrl_hook_installed = true
    log.info("[laser_dot_probe] Hooked LaserSightController.setPosition")
end
...
local function log_stage(stage)
    ...
    local pe_pos = get_pointer_effect_world_pos()
    ...
    -- 2026-08-15, resumed once more: SightEmitJointName = vfx_muzzle3 found
    -- via the LaserSightController dump -- the sight's own true emission
    -- joint, never checked before now. Logged raw so the same amplitude-
    -- ratio technique (yaw/pitch swing vs. camera's own swing) can be
    -- applied to it, same as was already done for muzzle_fwd/wrist_fwd/tip.
    local vfx3_pos, vfx3_fwd = get_vfx_muzzle3_ground_truth()
    local dev_deg_vfx3_vs_cam = (vfx3_fwd and cam_fwd) and angle_between_deg(vfx3_fwd, cam_fwd) or nil

    -- 2026-08-15, resumed once more: CreatedEffectContainer.get_effectPositions()
    -- polled live (not hooked -- three write hooks already confirmed dead).
    -- This is the actual VFX effect's own position -- very plausibly the real
    -- dot. Logged raw (world position) plus its direction from the muzzle
    -- for the same angle-vs-camera comparison as tip/muzzle/vfx3 above.
    local pe_dir = (pe_pos and muzzle_pos) and vec3_sub(pe_pos, muzzle_pos) or nil
    local dev_deg_pe_vs_cam = (pe_dir and cam_fwd) and angle_between_deg(pe_dir, cam_fwd) or nil

    log.info(string.format(
        "[laser_dot_probe] frame=%d stage=%-24s spine_on=%-5s spine_timing=%-7s src=%-28s tip=%s  muzzle_pos=%s  muzzle_fwd=%s  cam_fwd=%s  wrist_fwd=%s  vfx3_fwd=%s  pe_pos=%s  dev_deg=%s  dev_deg_vs_cam=%s  dev_deg_wrist_vs_cam=%s  dev_deg_tip_vs_cam=%s  dev_deg_vfx3_vs_cam=%s  dev_deg_pe_vs_cam=%s",
        ...
        dev_deg_vfx3_vs_cam and string.format("%.2f", dev_deg_vfx3_vs_cam) or "nil",
        dev_deg_pe_vs_cam and string.format("%.2f", dev_deg_pe_vs_cam) or "nil"))
end
...
-- Auto-fires dump_full_hierarchy() on the rising edge of aiming, so the
-- player never needs to touch the overlay while physically holding RG.
-- Clears itself after firing once, deliberately -- this is a one-shot
-- structural dump, not something that needs to repeat every aim.
local function check_auto_dump_hierarchy()
    if not state.auto_dump_hierarchy_on_aim then
        state.hierarchy_was_aiming = false
        return
    end
    local aiming = player_is_aiming()
    if aiming and not state.hierarchy_was_aiming then
        dump_full_hierarchy()
        dump_laser_sight_controller()
        dump_created_effect_container()
        dump_effect_positions_probe()
        state.auto_dump_hierarchy_on_aim = false
    end
    state.hierarchy_was_aiming = aiming
end
...
re.on_pre_application_entry("LateUpdateBehavior", function()
    ...
    check_auto_capture()
    check_auto_dump_hierarchy()
    try_install_setter_hook()
    try_install_stamp_hook()
    try_install_laser_sight_ctrl_hook()
...
re.on_draw_ui(function()
    ...
    imgui.text("LaserSightController.setPosition hook installed: " .. tostring(state.laser_sight_ctrl_hook_installed) ..
        " -- NEW lead (2026-08-15), logs during aim bursts / 10s captures")
    ...
    if imgui.button("Dump FULL weapon+player transform tree (all descendants) NOW") then
        dump_full_hierarchy()
    end
    imgui.same_line()
    if imgui.button("Dump LaserSightController NOW") then
        dump_laser_sight_controller()
    end
    imgui.same_line()
    if imgui.button("Dump PointerEffect (CreatedEffectContainer) NOW") then
        dump_created_effect_container()
    end
    if imgui.button("Probe get_effectPositions() shape NOW") then
        dump_effect_positions_probe()
    end
    local adc, adv = imgui.checkbox("Auto-dump ALL FOUR on next aim start (safer -- no button press needed while holding RG)", state.auto_dump_hierarchy_on_aim)
    if adc then state.auto_dump_hierarchy_on_aim = adv end
    imgui.text_colored(
        "Check the box, let go of the overlay, THEN aim normally -- fires itself the instant RG is held, then un-checks itself.",
        0xFF88CCFF)
    imgui.text_colored(
        "2026-08-15: found app.ropeway.LaserSightController on the 'LaserSight' child GameObject -- new lead, never found before.",
        0xFF88CCFF)
