...
---
--- 2026-08-13: two attempts were made and REVERTED this session to swap the
--- preference order here (details in re2_vr_real_weapon_grip_attempt
--- memory) based on the theory that raw controller position
--- (vrmod:get_position or vrc_manager.controllers_list[i].position) is the
--- "true" world position and __vr_lh_world's camera-relative reconstruction
--- (get_fp_style_hand_world_pos) merely "drifts". That theory was WRONG:
--- confirmed live both times that raw controller position is in VR
--- tracking/room space (near-origin values, e.g. -0.3,-0.1,-0.5) while the
--- anchor is in game-world space (e.g. 5.3,2.3,-5.3) -- completely
--- different coordinate systems, not comparable at all. The camera-relative
--- reconstruction isn't an approximation of world position, it IS the
--- (only) way to GET world position here -- it anchors the tracking-space
--- offset to the camera's real world position/orientation. Global-first is
--- back to being correct. Any head-orientation sensitivity in the grip
--- check is a real, separate, harder problem (the reconstruction's
--- yaw-only approximation) -- not something to "fix" by bypassing it.
...
--- 2026-08-13: returns a second value (the specific flag name) so a real
--- grip getting force-dropped by this can be diagnosed instead of guessed at
--- -- player reported the grip "snaps on and off" even while LG stays held,
--- and this function's checks run every frame BEFORE the LG state is even
--- looked at, so any single-frame true here force-releases an active grip
--- and resets the rising-edge tracker, requiring the real hand to happen to
--- be back within the tight grip radius the instant it clears. See
...
local function blocked_by_other_system()
    ...
    if real_blend > 0.001 then return true, "slide_dock_blend" end
    if rawget(_G, "__vr_mag_in_left_hand") == true then return true, "mag_in_left_hand" end
    if rawget(_G, "__vr_shell_in_left_hand") == true then return true, "shell_in_left_hand" end
    if rawget(_G, "__vr_in_holster_zone") == true then return true, "in_holster_zone" end
    if rawget(_G, "__vr_in_head_flashlight_zone") == true then return true, "in_head_flashlight_zone" end
    if rawget(_G, "__vr_block_left_support_in_mag_holster_zone") == true then return true, "block_left_support_in_mag_holster_zone" end
    return false, nil
end
...
re.on_frame(function()
    ...
    -- 2026-08-14: publish muzzle/anchor UNCONDITIONALLY once VR-active and
    -- weapon-covered checks pass -- moved above blocked_by_other_system()
    -- so re2_vr_recoil.lua's RG+LG latch (fully independent of this file's
    -- own engagement decision below, which the latch no longer reads at
    -- all) always has a valid weapon-relative anchor to render the docked
    -- hand from, even if THIS file's own engagement logic is momentarily
    -- blocked by another system (e.g. slide_dock_blend). The anchor is pure
    -- weapon-geometry (muzzle bone + fixed per-weapon offsets), unrelated to
    -- whatever this file's own grip decision is doing. See
    -- re2_vr_real_weapon_grip_attempt memory.
    ...
    local blocked, blocked_reason = blocked_by_other_system()
    if blocked then
        if grip.real_active then
            log.warn(string.format(
                "[re2_vr_cosmetic_dock] real grip force-dropped this frame -- blocked_by=%s",
                tostring(blocked_reason)))
        end
        set_real_grip(false, nil)
        grip.prev_lg = false
        return
...
-- 2026-08-14: collapsed behind a tree_node, off-screen by default -- these
-- sliders tune the weapon-relative grip anchor (get_weapon_support_grip_world_pos)
-- that re2_vr_recoil.lua's RG+LG support-hand latch still reads every frame for
-- its anchor-based left-hand placement once RG is released (see
-- patch_support_hand_ik_target), so kept fully live/functional, not deleted --
-- just not worth surfacing in the default UI since day-to-day play never needs
-- to touch them.
re.on_draw_ui(function()
    if not imgui.tree_node("Weapon grip tuning (advanced -- feeds the RG+LG support-hand latch anchor)") then return end
