...
re.on_draw_ui(function()
    ...
    local changed, new_val = imgui.checkbox("Enable auto-complete pickup screen (EXPERIMENTAL, may freeze the game)", state.enabled)
    if changed then
    ...
    imgui.text_colored(
        ...
        0xFF5050FF)
    imgui.text_colored(
        "NewInventoryDetailBehavior.open every time, even with auto-complete OFF.",
        0xFF5050FF)
    local changed_n, new_val_n = imgui.checkbox(
        "Make every pickup show the clean 'new item' card instead of the grid (SUSPECTED of causing freezes)",
