...
local function install_method_observers()
    ...
    local function hook_observer(type_def, method_name)
        ...
        pcall(function()
            sdk.hook(m,
                function(args)
                    -- 2026-08-14 fix: re.get_frame_count() was called
                    -- unguarded here and crashed on every single call to
                    -- this hook ("field 'get_frame_count' is not callable")
                    -- -- this codebase's OWN established pattern elsewhere
                    -- (re2_vr_haptics.lua, re2_vr_ik_extention.lua,
                    -- re2_vr_reload_ext_2.lua/_4.lua) always guards it with
                    -- `if re.get_frame_count then ... end` first, precisely
                    -- because it isn't guaranteed callable in every context
                    -- -- this hook skipped that guard. Frame number wasn't
                    -- essential to this diagnostic anyway, just dropped it
                    -- rather than reintroducing the same risk.
                    log.info("[pickup_cam_probe2] CALLED: " .. method_name)
