...
--
-- Snaps the rendered left hand onto the weapon's foregrip while LG is held,
-- and releases the instant LG is let go. It never lets the real hand
-- move/steer the weapon: the dock target is derived purely from the
-- weapon's own muzzle joint (offset back along the barrel axis), never from
-- where the left hand actually is.
--
-- 2026-08-11: switched from hand-proximity triggering to an LG gate, after
-- live VR testing found proximity unreliable -- inconsistent snap timing,
-- apparent dependence on gaze/camera-recenter, unreliable release
...
-- proximity-based holdout. Gated on LG alone, not LG+RG -- RG is normally
-- already held just for regular two-handed aiming, so requiring both would
-- make this active almost the entire time the player aims rather than a
-- distinct deliberate action.
--
...
re.on_frame(function()
    ...
    -- LG alone, not LG+RG -- RG is normally already held just for regular
    -- two-handed aiming (ready-to-fire), so gating on both would make this
    -- active almost the entire time the player aims, not a deliberate
    -- distinct gesture. LG by itself (support-grip button) is the actual
    -- deliberate "grab the foregrip" action. See file header for why this
    -- is a button gate at all rather than hand proximity.
    local want_on = is_left_grip_pressed()
...
log.info("[re2_vr_cosmetic_dock] Started. LG-gated cosmetic dock active for "
