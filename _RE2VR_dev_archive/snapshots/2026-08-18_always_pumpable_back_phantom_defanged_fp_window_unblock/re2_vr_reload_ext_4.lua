...
local gesture = {
    needs_pump = false,
    -- True only when the game itself put the weapon in a chamber-clear-
    -- needed state (fired with a shell left in the tube, or a shell just
    -- inserted into an emptied gun). False for a free/cosmetic rack the
    -- player starts by hand when the game doesn't require one. Gates
    -- whether complete_pump_cycle() calls the native chamber-clear/
    -- reload/eject finalization methods, so cosmetic racking never
    -- touches ammo state.
    pump_is_real = false,
    active = false,
    pull_done = false,
    ...
    -- Motion-pump lockout: set when a pump cycle completes, cleared by a
    -- fresh grip press or a real (game-required) pump arming. Without it
    -- the cosmetic gesture that auto-reactivates right after completion
    -- (grip still held) captures a rest-posture motion baseline, and
    -- ordinary aiming-hand drift then reads as a phantom full pull that
    -- arms needs_pump and blocks firing (seen live 2026-08-17: s drifted
    -- +14cm to ratio 1.00 within 1.3s of a completed cycle).
    mo_hold_off = false,
...
local function arm_pump_gesture(wp)
    ...
    -- Real pump required: motion drive is always allowed for it.
    gesture.mo_hold_off = false
...
local function complete_pump_cycle(wp_name)
    ...
    -- Always call this, cosmetic rack or not -- it's what tells the native
    -- weapon it's fire-ready again (see sync_chamber_shoot_ready() in
    -- re2_vr_reload_ext_1.lua for the same pattern), not something that
    -- only matters when ammo actually changed. Skipping it for cosmetic
    -- racks left the weapon stuck refusing to fire afterward.
    try_end_chamber_clear()
    local bp = get_pump_bind_pose(wp_name)
    set_pump_joint_local_z(bp.rest_z, bp)
    clear_manual_pump_state()
    -- AFTER the clear (which resets it false): motion drive stays cold
    -- until the player deliberately re-grips or a real shot re-arms the
    -- pump -- see the mo_hold_off comment in the gesture table.
    gesture.mo_hold_off = true
...
local function update_pump_support_slide_dock(wp_name)
    ...
    local grip = is_left_grip_pressed and is_left_grip_pressed() or false
    -- needs_pump OR active: the hand stays attached to the forend the
    -- whole time the grip is held. Requiring needs_pump alone dropped the
    -- hand the instant a cycle completed ("still lets go after pump
    -- handle goes back up", live 2026-08-18) -- with the cosmetic gesture
    -- auto-active while gripping, the no-pull FP-passthrough branch below
    -- hands the visual to the native support snap instead of releasing.
    local want_support = grip
        and (gesture.needs_pump or gesture.active)
...
local function update_manual_pump_gesture()
    ...
    --
    -- Allowed to start any time the grip is held, not only when the game
    -- has flagged needs_pump -- lets the player rack the pump freely
    -- (empty gun, or a live round still chambered) purely for feel.
    -- gesture.pump_is_real records whether the game actually required this
    -- cycle (vs. a free/cosmetic rack); used only to decide how to clean
    -- up if the grip is released before the cycle finishes (see the abort
    -- branch below). complete_pump_cycle() always finalizes the native
    -- chamber-clear/reload/eject state regardless -- that's what keeps the
    -- weapon fire-ready, not something tied to whether ammo changed. Ammo
    -- itself is never touched by a cosmetic rack simply because no real
    -- fire/insert event triggered it in the first place.
    if grip then
        if not gesture.active then
            if not gesture.pump_await_grip_release then
                -- A FRESH grip press (edge, not the continued hold that
                -- auto-reactivates right after a completed cycle) is a
                -- deliberate re-grab: re-enable the motion drive.
                if not gesture.last_grip then
                    gesture.mo_hold_off = false
                end
                gesture.pump_is_real = gesture.needs_pump == true
                if not gesture.needs_pump then
                    -- Free/cosmetic grip: prime the bind pose/pull-axis
                    -- setup (same as arm_pump_gesture() would do) so the
                    -- pump joint and hand-follow IK are ready, but do NOT
                    -- set needs_pump yet -- this grip button doubles as the
                    -- normal two-handed aim/support hold, so merely
                    -- gripping (without ever pulling the trigger to rack)
                    -- must not block firing. needs_pump is armed later, at
                    -- the trig_pressed_edge commit point below, only once
                    -- the player actually starts pulling.
                    gesture.cached_wp = wp
                    local bp0 = get_pump_bind_pose(wp)
                    if bp0 then
                        gesture.grab_bind_z = bp0.parked_z
                        set_pump_joint_local_z(bp0.parked_z, bp0)
                        prepare_pump_axis_for_weapon(wp)
                    end
                end
                gesture.active = true
                ...
                gesture.pump_committed = false
                gesture.trigger_prev = false
                -- Fresh grip = fresh motion-pump baseline (relative-hands
                -- pull is measured against where the hands are right now).
                gesture.mo_init = false
                gesture.mo_ratio = 0.0
        ...
        if gesture.active then
            ...
            if trig_pressed_edge and not gesture.pull_done then
                ...
                if not gesture.pump_is_real and not gesture.needs_pump then
                    -- Cosmetic cycle: only now, on actual commit to pull,
                    -- arm needs_pump -- blocks fire for the rest of this
                    -- rack motion, same as a real cycle, but never merely
                    -- from resting a hand on the grip.
                    gesture.needs_pump = true
                end
            end
            ...
            -- Motion drive: real relative hand motion pumps the forend,
            -- layered on top of the LT drive above (whichever is further
            -- back wins, so LT keeps working unchanged). Moving the LEFT
            -- hand back, pushing the RIGHT hand (the gun) forward, or
            -- both at once all register as the same pull -- see
            -- M.gesture_motion_ratio in ext_2 for the mechanism and the
            -- sign-locking that prevents the original mod's inverted
            -- front-to-back pump readings.
            local mo = nil
            -- Motion pumping is ALWAYS available (player's explicit call,
            -- 2026-08-18: "i really really don't wanna give that up" --
            -- reverts the one-session real-pumps-only experiment). Phantom
            -- protection is layered elsewhere instead: mo_hold_off after
            -- each completed cycle (re-squeeze LG to free-pump again --
            -- the mechanism the player confirmed working), and a
            -- motion-only cosmetic pull never arms needs_pump (below), so
            -- a phantom can animate the forend at worst -- it can never
            -- block firing.
            if not gesture.mo_hold_off
                and mp_cfg().motion_pump_enabled ~= false
                and slide_gesture and slide_gesture.gesture_motion_ratio then
                mo = slide_gesture.gesture_motion_ratio(gesture, wp, pump_gesture_ctx(),
                    tonumber(mp_cfg().motion_pull_scale) or 1.0,
                    tonumber(mp_cfg().motion_deadzone_m) or 0.015)
                if mo and mo > gesture.pump_travel then gesture.pump_travel = mo end
            end
            ...
            if not gesture.pull_done
                and (gesture.pump_committed or (mo ~= nil and mo >= 1.0 - 1e-4))
                and gesture.pump_travel >= 1.0 - 1e-4 then
                -- A motion pull that physically reached full travel is a
                -- committed pump, no LT needed. For a cosmetic (not
                -- game-required) motion cycle, arm needs_pump only here at
                -- the full-pull point -- NOT earlier at partial pulls --
                -- so incidental two-handed-aim hand drift can never block
                -- firing; only a genuinely completed pull-down does, and
                -- the return stroke then clears it via complete_pump_cycle
                -- exactly like the LT path.
                -- LT-committed cosmetic racks still arm needs_pump (an LT
                -- press is deliberate). A MOTION-only cosmetic pull never
                -- does: that arming is exactly how a drifted-baseline
                -- phantom pull became a fire block (live log 2026-08-18
                -- 00:34, ratio pinned 0.75-1.00 for 6s while just holding
                -- the gun). Without it a phantom costs a spurious forend
                -- animation at worst -- firing is never blocked, and the
                -- pull-arm IK dock (needs_pump-gated) never yanks the arm.
                if gesture.pump_committed
                    and not gesture.pump_is_real and not gesture.needs_pump then
                    gesture.needs_pump = true
                end
    ...
    elseif gesture.active and not grip then
        ...
        if not gesture.pump_is_real then
            -- Cosmetic-only rack: fully clear (including needs_pump) so an
            -- abandoned free rack doesn't leave the weapon looking like it
            -- still needs a real pump afterward.
            clear_manual_pump_state()
            local bp = get_pump_bind_pose(wp)
...
function Pump.update_globals()
    ...
    -- `not gesture.active`: never block the FP left hand while the player
    -- is actually holding the forend (grip held => active). Without this,
    -- finishing a pump INSIDE the 2.5s post-shot suppress window
    -- (pump_window_sec) blocked FP hand IK the moment needs_pump went
    -- false -- the hand visibly floated off the forend for the window's
    -- remainder ("still loses that grip", live 2026-08-18; fast motion
    -- pumps finish well inside the window, slow LT pumps mostly ate it,
    -- which is why this never stood out before).
    rawset(_G, "__vr_block_empty_pump_reload_motion",
        want and pump_window_active() and not gesture.active)
    publish_gesture_globals(get_weapon_go_name())
end
