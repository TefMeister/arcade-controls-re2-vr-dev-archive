...
local function world_look_delta(from_dir, to_dir, weight)
    ...
    local angle = math.acos(dot) * (weight or 1.0)
    if angle < 1e-5 then return Quaternion.identity() end
    local axis = vec3_cross(a, b)
    local axis_len = math.sqrt(axis.x * axis.x + axis.y * axis.y + axis.z * axis.z)
    if axis_len < 1e-6 then return Quaternion.identity() end
...
--- instead of just a cosmetic hand placement with no effect on aim.
--- Ground-truth muzzle position/forward comes from
--- __vr_muzzle_world_pos/__vr_muzzle_world_fwd (published by that same
--- script every frame a covered weapon is equipped) -- one frame behind
--- since it's read before this frame's IK solve runs, same as recoil's own
--- frame-cached values, stable enough for a smooth continuous blend. Keeps
--- rh_post's position untouched, only rotates it. Falls back to rh_post
--- completely unmodified on ANY failure -- this must never be able to
--- corrupt the matrix or crash, since it runs inside the IK pre-hook.
local function blend_aim_toward_left_hand(rh_post)
    if rh_post == nil then return rh_post end
    local ok, result = pcall(function()
        local muzzle_pos = rawget(_G, "__vr_muzzle_world_pos")
        local muzzle_fwd = rawget(_G, "__vr_muzzle_world_fwd")
        if not vec3_valid(muzzle_pos) or not vec3_valid(muzzle_fwd) then return rh_post end

        local lh = get_hand_world_pos("left")
        if not vec3_valid(lh) then return rh_post end

        local desired_fwd = Vector3f.new(lh.x - muzzle_pos.x, lh.y - muzzle_pos.y, lh.z - muzzle_pos.z)
        local weight = clampf(tonumber(CFG.two_hand_aim_blend) or 0.85, 0.0, 1.0)
        local delta = world_look_delta(muzzle_fwd, desired_fwd, weight)
        if not delta then return rh_post end

        local cur_rot = rh_post:to_quat():normalized()
        local new_rot = (delta * cur_rot):normalized()
        local new_m = new_rot:to_mat4()
        local p = rh_post[3]
