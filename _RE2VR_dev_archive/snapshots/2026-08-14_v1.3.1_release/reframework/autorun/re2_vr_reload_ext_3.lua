...
refresh_weapon_refs = function()
    ...
    if reuse_rest then
        ...
        if not pos then return false end
        -- Small per-weapon nudge on top of the raw joint position -- only
        -- relevant for weapons using bullet_joint_by_wp's fallback path
        -- (NOT per-chamber-joint weapons like SLS60, which resolve target
        -- via get_hand_bullet_node_name's chamber-node branch instead --
        -- see mag_exit_by_wp for SLS60's actual tunable dock point).
        local dock_off = type(CFG.bullet_dock_offset_by_wp) == "table" and CFG.bullet_dock_offset_by_wp[wp]
        rv.rest_x = pos.x + (dock_off and tonumber(dock_off.x) or 0.0)
        rv.rest_y = pos.y + (dock_off and tonumber(dock_off.y) or 0.0)
        rv.rest_z = pos.z + (dock_off and tonumber(dock_off.z) or 0.0)
        if rot then
            rv.rest_w = rot.w
            rv.rest_rx = rot.x
            rv.rest_ry = rot.y
...
local function begin_bullet_insert_anim(wp)
    ...
    -- Weapons with per-chamber joints (SLS60-style, get_bullet_chamber_nodes
    -- ~= nil) need the animation to land ON the actual chamber -- rv.rest_x/y/z,
    -- the joint's true rest position captured at refresh time -- not at
    -- mag_exit, which is the ergonomic hand-reach/grab-detection point
    -- (tuned separately, see mag_exit_by_wp) and generally isn't anywhere
    -- near the chamber hole itself. Other weapons (mag-fed, no chamber
    -- nodes) keep using mag_exit, since for those it's already at/near the
    -- real insertion point.
    local exit
    if get_bullet_chamber_nodes(wp) then
        exit = Vector3f.new(rv.rest_x, rv.rest_y, rv.rest_z)
    else
        exit = get_mag_exit_local(wp)
    end
    rv.slide_lx0 = cur.x
    rv.slide_ly0 = cur.y
    rv.slide_lz0 = cur.z
