...
-- Equipment.ForceEquipType every single frame using the same Nullable
-- struct-field-write technique proven reliable in the sub-weapon-trigger
-- feature (see re2_vr_holster.lua), overriding whatever the native
-- support-hold logic just tried to do. Once
...
-- LG does afterward.
--
-- 2026-08-12: a SECOND, independent suppression trigger was added --
-- re2_vr_cosmetic_dock.lua's __vr_real_grip_active, set when LG grips a
-- weapon's foregrip for real two-handed holding (regardless of RG order or
-- even whether RG is held at all). Combined with the RG-order path above
-- into one `suppress_active` flag before driving force/clear, rather than
-- two separate state machines -- they'd otherwise race over the same
-- pending_clear_* variables (e.g. RG releasing while a real grip is still
-- held would wrongly schedule a clear and let the sub-weapon reappear).
...
local last_suppress_active = false
...
re.on_frame(function()
    ...
    if not right_grip and last_right_grip then
        suppress_this_rg_hold = false
    end
    last_right_grip = right_grip

    -- 2026-08-12: a real two-handed weapon grip (re2_vr_cosmetic_dock.lua's
    -- __vr_real_grip_active, engaged by LG at the foregrip regardless of RG
    -- order) must ALSO suppress the sub-weapon, same as the RG-order path
    -- above -- combined into one flag rather than two independent
    -- force/clear state machines, since both would otherwise fight over the
    -- same pending_clear_* variables (e.g. RG releasing while a real grip
    -- is still held would wrongly schedule a clear).
    local real_grip_active = rawget(_G, "__vr_real_grip_active") == true
    local suppress_active = (right_grip and suppress_this_rg_hold) or real_grip_active
    ...
    elseif last_suppress_active then
        log.info("[re2_vr_suppress_supporthold] suppression ended, scheduling clear")
        local equipment = get_equipment()
        if equipment then
    ...
    last_suppress_active = suppress_active
