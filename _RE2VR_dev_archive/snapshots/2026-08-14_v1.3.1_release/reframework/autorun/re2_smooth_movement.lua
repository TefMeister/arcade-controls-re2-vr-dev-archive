...
-- Same get_IsHold()-based aim check used elsewhere in this mod (re2_vr_
-- recoil.lua/re2_vr_haptics.lua/re2_vr_ik_extention.lua). Native root-motion
-- movement is throttled while aiming/gripping a weapon two-handed (RG
-- held, weapon out) -- this script normally just measures that native
-- per-frame movement and redirects it toward the joystick direction, so it
-- silently inherited that same slowdown. AIM_SPEED_OVERRIDE_MPS below
-- bypasses it so two-handing a weapon doesn't force a walk.
...
local function player_is_aiming()
    local cond = get_survivor_condition(re2.player)
    if not cond then return false end
    local ok, aiming = pcall(function() return cond:call("get_IsHold") end)
    return ok and aiming == true
end

-- Reference run pace (m/s) used while aiming, in place of the throttled
-- native value. Tune this based on how it feels in testing.
local AIM_SPEED_OVERRIDE_MPS = 1.3
...
re.on_application_entry("UpdateMotion", function()
    ...
    if cfg.movement_stabilization then
        ...
        if has_valid_player_position then
            ...
            delta.y = 0.0
            local observed_speed = delta:length()

            -- cap the speed to something that won't balloon out of control.
            observed_speed = math.min(observed_speed, 1.0)
            ...
            if player_is_aiming() then
                -- Bypass the throttled native-observed magnitude entirely
                -- while aiming -- drive speed from raw stick input at a
                -- fixed reference pace instead. Deliberately does not
                -- touch last_ema_speed, so the EMA resumes cleanly (no
                -- jump/lag) once aiming ends.
                local stick_mag = math.min(axis_l:length(), 1.0)
                speed = stick_mag * AIM_SPEED_OVERRIDE_MPS * delta_t
            else
                local speed_normalized = observed_speed / delta_t
