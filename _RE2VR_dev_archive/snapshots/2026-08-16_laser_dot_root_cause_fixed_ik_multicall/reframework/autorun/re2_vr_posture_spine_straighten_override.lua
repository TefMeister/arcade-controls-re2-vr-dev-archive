...
local function apply_override(player)
    ...
    local raw_fade = state.speed_fade_enabled and speed_fade_multiplier(speed, turn_deg_s) or 1.0
...
re.on_draw_ui(function()
    ...
    local ucc, ucv = imgui.checkbox("Also freeze spine_1/spine_2 (whole upper torso, legs unaffected)", state.correct_upper_chain)
    if ucc then state.correct_upper_chain = ucv end

    local pcc, pcv = imgui.checkbox("Also freeze hips ROTATION (was 'pelvis' -- wrong name, fixed 2026-08-16)", state.correct_pelvis_rotation)
    if pcc then state.correct_pelvis_rotation = pcv end

    local lcc, lcv = imgui.checkbox("DIAGNOSTIC: also freeze legs (WILL look broken -- just to isolate the cause)", state.correct_legs_diagnostic)
    if lcc then state.correct_legs_diagnostic = lcv end

    local rcc, rcv = imgui.checkbox("Also freeze root/Null_Offset/COG rotation (untested -- caution, may affect facing)", state.correct_root_chain)
    if rcc then state.correct_root_chain = rcv end

    if imgui.button("Dump ALL joint names to log (diagnostic, for finding real leg joint names)") then
    ...
    imgui.spacing()
    local sfc, sfv = imgui.checkbox("Turn correction off while moving/turning (fixes running shake)", state.speed_fade_enabled)
    ...
    imgui.text(string.format("Current speed: %.2f m/s -- turn rate: %.0f deg/s -- fade: %.0f%% -- effective strength: %.2f",
        state.last_speed_mps, state.last_turn_deg_s, state.last_speed_fade * 100.0, state.strength * state.last_speed_fade))
