...
local function log_stage(stage)
    ...
    log.info(string.format(
        "[laser_dot_probe] frame=%d stage=%-24s spine_on=%-5s src=%-28s tip=%s  muzzle_pos=%s  muzzle_fwd=%s  cam_fwd=%s  wrist_fwd=%s  dev_deg=%s  dev_deg_vs_cam=%s  dev_deg_wrist_vs_cam=%s  dev_deg_tip_vs_cam=%s",
        state.frame, stage, tostring(spine_on == true), tostring(muzzle_src),
        fmt_vec(tip),
        fmt_vec(muzzle_pos),
        fmt_vec(muzzle_fwd),
        fmt_vec(cam_fwd),
