...
--- Ground-truth muzzle position/forward, republished every frame a covered
--- weapon is equipped so re2_vr_recoil.lua can blend aim toward the real
--- left hand without duplicating the MuzzleJoint reflection lookup.
local function publish_muzzle_globals(muzzle_pos, muzzle_fwd)
...
re.on_frame(function()
    if not vr_active() or blocked_by_other_system() then
        publish_muzzle_globals(nil, nil)
    ...
    local muzzle_pos, muzzle_fwd = get_weapon_muzzle_pos_fwd()
    publish_muzzle_globals(muzzle_pos, muzzle_fwd)
    ...
    if lg and not grip.prev_lg then
        local anchor = get_weapon_support_grip_world_pos(wp_id, get_active_profile())
        local real_lh = get_real_left_hand_pos()
        local radius = clampf(tonumber(CFG.grip_zone_radius_m) or 0.18, 0.05, 0.40)
        local close_enough = vec3_valid(anchor) and vec3_valid(real_lh) and vec3_dist(anchor, real_lh) <= radius
...
re.on_draw_ui(function()
    ...
    local changed_radius, new_radius = imgui.slider_float(
        "Grip zone radius (m)", tonumber(CFG.grip_zone_radius_m) or 0.18, 0.05, 0.40, "%.3f")
