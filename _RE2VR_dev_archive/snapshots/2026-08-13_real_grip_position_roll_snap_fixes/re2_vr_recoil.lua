...
local function blend_aim_toward_left_hand(rh_post)
    ...
    local ok, result = pcall(function()
        ...
        local jump_deg = (prev_angle ~= nil and prev_frame ~= nil and frame_id == prev_frame + 1)
            and math.abs(angle_deg - prev_angle) or 0.0
        if angle_deg > 90.0 or jump_deg > 45.0 then
            local now = uptime_now()
            if (frame_ik.grip_dbg_last_log_time or 0) + 0.1 <= now then
                frame_ik.grip_dbg_last_log_time = now
                log.warn(string.format(
                    "[re2_vr_recoil] GRIP_DEBUG frame=%d angle=%.1fdeg axis_len=%.5f jump=%.1fdeg weight=%.2f muzzle_fwd=(%.3f,%.3f,%.3f) desired_fwd=(%.3f,%.3f,%.3f)",
...
local function patch_support_hand_ik_target(arm_fit_data, field_name, lh_pre_matrix)
    local rh_pre, pos_rel, rot_rel = get_weapon_hand_pre_for_support(lh_pre_matrix)
    ...
    return write_patched_target_matrix(arm_fit_data, field_name, lh_post)
end
...
local function patch_ik_target_matrix(arm_fit_data, wrist_side)
    ...
    -- 2026-08-12: real two-handed grip (re2_vr_cosmetic_dock.lua's
    -- __vr_real_grip_active) reverses the usual relationship -- normally
    -- the LEFT hand is patched to follow the weapon (see
    -- should_support_hand_follow_weapon below); while a real grip is
    -- engaged, the WEAPON's aim blends toward the real left hand instead,
    -- and the left hand itself is left completely untouched so it shows
    -- real tracked position, not a cosmetic placement.
    local real_grip = is_real_grip_active()

    if wrist_side == "right" then
        frame_ik.rh_ik_pre_matrix = target_matrix
        local rh_post = build_recoiled_hand_matrix(target_matrix)
        if real_grip then
            rh_post = blend_aim_toward_left_hand(rh_post)
        elseif rh_post ~= nil and frame_ik.lh_pre_matrix ~= nil and frame_ik.lh_arm_fit_data ~= nil then
    ...
    if wrist_side == "left" then
        if real_grip then
            -- 2026-08-13: soft visual snap toward the grip anchor while a
            -- real grip is held, purely for rendering feel -- the AIM math
            -- in blend_aim_toward_left_hand still reads the raw tracked
            -- left hand position (get_hand_world_pos) untouched, never this
            -- blended pose, so a visually "stuck" hand can never desync
            -- what's actually driving where the gun points. Blend factor is
            -- proximity-based off the REAL tracked hand's current distance
            -- from the anchor (not the rendered pose), so it fades out
            -- cleanly if the hand moves away after engaging rather than
            -- permanently gluing the hand there for the whole hold. Reuses
            -- the exact same write path (write_patched_target_matrix on the
            -- left wrist's arm-fit data) that the RG-driven "support hand
            -- follows weapon" system below already uses and is confirmed to
            -- visibly move the rendered hand -- see
            -- re2_vr_real_weapon_grip_attempt memory. Falls through to the
            -- untouched native pose (real hand shows through, as before) if
            -- anything here is unavailable or degenerate -- must never
            -- crash or produce a wrong-looking lock.
            local ok_snap, handled = pcall(function()
                local anchor_pos = rawget(_G, "__vr_grip_anchor_world_pos")
                local anchor_rot = rawget(_G, "__vr_grip_anchor_world_rot")
                local real_lh = get_hand_world_pos("left")
                if not vec3_valid(anchor_pos) or anchor_rot == nil or not vec3_valid(real_lh) then
                    return false
                end
                -- 2026-08-13: was `if t <= 0.001 then return false end` here,
                -- falling through to the completely untouched native pose at
                -- low t -- that meant the rendered hand could jump between
                -- TWO DIFFERENT pose sources (this reconstructed blend vs.
                -- whatever native/untouched actually is) every time ordinary
                -- hand-tracking jitter crossed the snap_radius_m boundary,
                -- which is exactly the "swaps between two locations very
                -- rapidly" a player reported. Fixed: always write the
                -- reconstructed pose, never fall through -- at t=0 it
                -- already equals (real_lh, real_rot-or-anchor_rot), which is
                -- continuous with every t>0 value, so there's no second pose
                -- source left to flicker against.
                local snap_radius_m = 0.22
                local t = clampf(1.0 - (vec3_dist(real_lh, anchor_pos) / snap_radius_m), 0.0, 1.0)
                local _, real_rot = matrix_to_pos_rot(target_matrix)
                local blended_pos = Vector3f.new(
                    real_lh.x + (anchor_pos.x - real_lh.x) * t,
                    real_lh.y + (anchor_pos.y - real_lh.y) * t,
                    real_lh.z + (anchor_pos.z - real_lh.z) * t)
                local blended_rot = anchor_rot
                if real_rot ~= nil then
                    local ok1, r1 = pcall(function() return real_rot:slerp(anchor_rot, t) end)
                    if ok1 and r1 then
                        blended_rot = r1
                    else
                        local ok2, r2 = pcall(function() return real_rot:nlerp(anchor_rot, t) end)
                        if ok2 and r2 then blended_rot = r2 end
                    end
                end
                local snapped_m = build_matrix_from_pos_rot(blended_pos, blended_rot)
                if snapped_m == nil then
                    return false
                end
                return write_patched_target_matrix(arm_fit_data, field_name, snapped_m)
            end)
            if ok_snap and handled then
                return true
            end
            return false
        end
...
local function on_pre_update_ik(args)
    ...
    -- 2026-08-12: also let a real two-handed grip through this gate even with zero
    -- recoil -- blend_aim_toward_left_hand needs to run every frame the grip is held
    -- (continuous aim tracking), not just the brief window right after a shot while
    -- the recoil spring is still animating. Without this, the whole IK patch (and the
    -- aim blend inside it) was only ever reached for a few frames after firing, which
    -- is why the weapon didn't visibly follow the hand during normal holding/aiming.
    if not frame_apply.has_recoil and not is_real_grip_active() then
