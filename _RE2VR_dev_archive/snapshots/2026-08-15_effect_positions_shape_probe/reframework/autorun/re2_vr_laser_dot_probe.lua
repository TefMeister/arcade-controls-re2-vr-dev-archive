...
local function get_pointer_effect_world_pos()
    ...
    local positions = safe(function() return container:call("get_effectPositions") end)
    if not positions then return nil end

    -- shape 1: positions is a list wrapper -- get_elements() sugar method
    local elements = safe(function() return positions:get_elements() end)
    if elements and #elements > 0 then
        return elements[1]
    end
    -- shape 2: positions IS already a single vec3-like value
    if type(positions) == "table" and positions.x ~= nil then
        return positions
    end
    return nil
end
