...
-- (2026-08-09) The run-toggle fix's setInhibitPetient approach doesn't work --
-- it only blocks/permits the JOG petient order, it can't request one, and
-- nothing in the VR control path requests one. Found via a reflection dump
-- (re2_vr_run_precede_bits_probe.lua) that PlayerActionOrderer itself has a
-- direct get_JogMode/set_JogMode pair -- a much more promising lever, but
-- unknown meaning (could be "is currently jogging", or could just mirror the
-- Toggle/Hold/Always Run Type menu setting). Observing only, same
-- never-write-blind lesson as the set_IsJog freeze above, before ever
-- calling set_JogMode ourselves.
local function get_action_orderer(cond)
    return safe(function() return cond:call("get_ActionOrderer") end)
end

local last_jog_mode = nil
local function poll_jog_mode(ao)
    if not ao then return end
    local ok, v = pcall(function() return ao:call("get_JogMode") end)
    if not ok or v == nil then return end
    if last_jog_mode == nil then
        log.info("[run_state_probe] ActionOrderer.JogMode EXISTS, initial value = " .. tostring(v))
        last_jog_mode = v
    elseif last_jog_mode ~= v then
        log.info(string.format("[run_state_probe] ActionOrderer.JogMode changed: %s -> %s", tostring(last_jog_mode), tostring(v)))
        last_jog_mode = v
    end
end

local jog_mode_hook_installed = false
local function install_jog_mode_hook(ao)
    if jog_mode_hook_installed or not ao then return end
    local td = safe(function() return ao:get_type_definition() end)
    if not td then return end
    local m = find_method_up_hierarchy(td, "set_JogMode")
    if not m then
        log.warn("[run_state_probe] could not find set_JogMode to hook")
        return
    end
    jog_mode_hook_installed = true
    sdk.hook(m,
        function(args)
            local val = "?"
            local ok, v = pcall(function() return sdk.to_int64(args[3]) end)
            if ok then val = tostring(v) end
            log.info(string.format("[run_state_probe] NATIVE CALL set_JogMode(%s)", val))
            return sdk.PreHookResult.CALL_ORIGINAL
        end,
        function(retval) return retval end -- observe only, never alter the return value
    )
    log.info("[run_state_probe] Installed set_JogMode observation hook")
end
...
re.on_draw_ui(function()
    ...
    imgui.text_colored(
        ...
        0xFFFF8888)
    imgui.text_colored(
        "New: watching ActionOrderer.JogMode (get_/set_) -- observe only, not written to yet.",
        0xFF88CCFF)
    imgui.text("ActionOrderer.JogMode: " .. tostring(last_jog_mode))
end)
