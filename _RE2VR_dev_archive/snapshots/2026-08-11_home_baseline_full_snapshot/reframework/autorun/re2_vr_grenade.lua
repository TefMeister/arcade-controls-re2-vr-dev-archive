...
re.on_application_entry("BeginRendering", function()
    ...
    if re2.weapon_gameobject ~= nil then
        local ok_tf, weapon_transform = pcall(function() return re2.weapon_gameobject:call("get_Transform") end)
        if ok_tf and weapon_transform ~= nil then
            local ok_pos, pos = pcall(function() return weapon_transform:call("get_Position") end)
            if ok_pos and pos ~= nil then last_grenade_position = pos end
            local ok_rot, rot = pcall(function() return weapon_transform:call("get_Rotation") end)
            if ok_rot and rot ~= nil then last_grenade_rotation = rot end
        end
    end
end)
