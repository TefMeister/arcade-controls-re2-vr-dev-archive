...
-- master toggle off, narrowing the search -- see `force_new_item_look`
-- below, now also disabled by default and the prime suspect (it's the one
-- piece of this file that ran completely unconditionally regardless of this
-- `enabled` flag).
--
...
local state = {
    ...
    -- 2026-08-14: flipped back to DISABLED after a second confirmed herb-
    -- combine freeze with the master auto-complete toggle OFF -- this hook
    -- is the ONE thing in this file that runs completely unconditionally
    -- (install_open_mode_rewrite_hook is installed every frame regardless of
    -- `enabled`, see the on_frame loop below), so it was still active and
    -- rewriting every single NewInventoryDetailBehavior.open call even with
    -- auto-complete off. Prime suspect: re2_vr_run_state_probe.lua's own
    -- header documents a prior CONFIRMED full game hang in this exact
    -- project from forcing a different field (IsFinish) on this same
    -- NewInventoryDetailBehavior class -- forcing state/flow-control values
    -- on this specific native object has a known history of hanging the
    -- game. Not proven as THE cause yet, but it's the only unconditional
    -- mod behavior touching the screen that froze, so it's off until tested
    -- in isolation. Confirmed working (visually) since 2026-08-10, but that
    -- only verified the card display looked right, never ruled out a hang
    -- risk under specific conditions like a merge result.
...
re.on_draw_ui(function()
    ...
    imgui.text_colored(
        "PRIME SUSPECT for the herb-combine freeze -- unconditionally rewrites",
        0xFF5050FF)
    imgui.text_colored(
        "NewInventoryDetailBehavior.open every time, even with [APU] above OFF.",
        0xFF5050FF)
    imgui.text_colored(
        "This checkbox is fully independent of [APU] in THIS SCRIPT's own code --",
        0xFF88CCFF)
    imgui.text_colored(
        "on its own it never arms do_grant_step/close/finish (all gated on [APU]",
        0xFF88CCFF)
    imgui.text_colored(
        "above). If an item still lands with zero extra clicks with ONLY this on,",
        0xFF88CCFF)
    imgui.text_colored(
        "that's native RE2 behavior from the game's own 'new item' screen mode,",
        0xFF88CCFF)
    imgui.text_colored(
        "not this script -- worth knowing when judging whether this is cosmetic-only.",
        0xFF88CCFF)
    local changed_n, new_val_n = imgui.checkbox(
        "Make every pickup show the clean 'new item' card instead of the grid [NIC] (SUSPECTED of causing freezes)",
