...
local state = {
    ...
    -- 2026-08-14: DECOUPLED from APU (triggers/restores off its own
    ...
    -- and the on_frame loop below) then LIVE-TESTED in isolation, confirmed
    -- from the log the write genuinely took effect (e.g. "Suppressed detail
    -- blur/zoom (was scale=20.0 mip=3.0 detailFov=60.0, normalFov=40.0)",
    -- correct restore on close) -- and the VR black screen was STILL
    -- unchanged. DEAD END: DetailBlurScale/DetailBlurMipLevel/DetailCameraFov
    -- are not what's actually producing the black background, same class of
    -- dead end as re2_vr_inventory_bg_tint_status's BgBlur/ColorScale
    -- findings (numeric write succeeds and reads back correctly, zero visual
    -- effect) -- whatever's really driving this is very likely shader/
    -- render-level, not reachable through these reflected properties.
    -- Left in place (not deleted) since it's a real, harmless toggle and the
    -- decoupling work is still valid infrastructure, but don't expect it to
    -- fix the black screen -- see re2_vr_pickup_reuse_itembox_camera_idea
    -- memory for what's being tried next.
    suppress_blur = false,
    original_blur = nil,
    -- 2026-08-14: next attempt in the black-screen investigation, after
    -- three rounds of reflection probing confirmed the camera object itself
    -- is legitimate (sits right at the player's own position, sane FOV/
    -- projection, not a debug camera) and numeric blur/FOV fields are
    -- provably inert (see suppress_blur above and
    -- re2_vr_inventory_bg_tint_status memory -- same dead end twice). The
    -- one remaining real lead: NewInventoryBehavior.activatePostEffectCapture
    -- is a genuine trigger method (not a numeric parameter) that fires
    -- exactly once when the detail screen opens -- confirmed live via
    -- re2_vr_pickup_camera_investigation_probe2.lua. This toggle SKIPS that
    -- one native call entirely (PreHookResult.SKIP_ORIGINAL) while leaving
    -- everything else -- openDetailDisp, open() itself, the manual
    -- confirm/grid flow, setItemCamera's per-frame camera drive, close/
    -- deactivatePostEffectDetail -- completely untouched. Deliberately more
    -- surgical than skipping open() entirely: pickup MECHANICS (data,
    -- manual confirmation, closing) stay bit-for-bit identical either way --
    -- only this one render-side trigger call is suppressed. Genuinely
    -- untested -- unknown whether this actually stops the black screen, or
    -- whether it causes some OTHER visual side effect (the 3D item model
    -- itself might render oddly without whatever this effect normally
    -- also sets up). Off by default.
    skip_post_effect_capture = false,
...
-- 2026-08-14: skip_post_effect_capture experiment -- see the state field's
-- own comment for the full reasoning. PRE-hook only, SKIP_ORIGINAL when the
-- toggle is on, so the native activatePostEffectCapture body never runs at
-- all for that call. No POST-hook needed since there's nothing to observe
-- when the call is skipped, and CALL_ORIGINAL's own return value is passed
-- through unchanged when the toggle is off.
local hooked_post_effect_capture = false
local function install_post_effect_capture_hook()
    if hooked_post_effect_capture then return end
    local inv_type = sdk.find_type_definition(NS("gui.NewInventoryBehavior"))
    if not inv_type then return end
    local m = inv_type:get_method("activatePostEffectCapture")
    if not m then
        log_line("could not find NewInventoryBehavior.activatePostEffectCapture")
        return
    end
    local ok_h = pcall(function()
        sdk.hook(m,
            function(args)
                if state.skip_post_effect_capture then
                    log_line("activatePostEffectCapture SKIPPED (skip_post_effect_capture on)")
    ...
    if ok_h then
        hooked_post_effect_capture = true
        log_line("Hooked NewInventoryBehavior.activatePostEffectCapture")
    end
end
...
re.on_frame(function()
    ...
    if not hooked_post_effect_capture then
        pcall(install_post_effect_capture_hook)
    end
...
re.on_draw_ui(function()
    ...
    imgui.text_colored(
        "CONFIRMED DEAD END (2026-08-14): write verified taking effect via log",
        0xFFAAAAAA)
    imgui.text_colored(
        "readback, VR black screen unchanged regardless. Left here as a harmless",
        0xFFAAAAAA)
    imgui.text_colored(
        "toggle, not expected to fix anything -- don't re-test this angle blind.",
        0xFFAAAAAA)
    local changed_b, new_val_b = imgui.checkbox(
        "Suppress detail blur/zoom [BG] (CONFIRMED no effect on the VR black screen)",
        state.suppress_blur)
    if changed_b then
    ...
    imgui.spacing()
    imgui.text_colored(
        "UNTESTED experiment (2026-08-14): skips NewInventoryBehavior.",
        0xFFCCAA55)
    imgui.text_colored(
        "activatePostEffectCapture entirely for the pickup detail screen -- the",
        0xFFCCAA55)
    imgui.text_colored(
        "actual trigger method (not a numeric knob) for whatever effect is",
        0xFFCCAA55)
    imgui.text_colored(
        "producing the VR black screen. Pickup mechanics (data, manual confirm,",
        0xFFCCAA55)
    imgui.text_colored(
        "closing) are untouched either way -- only this one render-side call is",
        0xFFCCAA55)
    imgui.text_colored(
        "skipped. Unknown side effects -- watch the 3D item model itself too.",
        0xFFCCAA55)
    local changed_pe, new_val_pe = imgui.checkbox(
        "Skip detail screen post-effect capture [BG2] (EXPERIMENTAL, untested)",
        state.skip_post_effect_capture)
    if changed_pe then
        state.skip_post_effect_capture = new_val_pe
        log_line("Skip post-effect capture [BG2] = " .. tostring(new_val_pe))
    end
