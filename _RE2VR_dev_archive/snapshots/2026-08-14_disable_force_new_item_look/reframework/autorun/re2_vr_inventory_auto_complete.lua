...
-- this feature active (root cause NOT confirmed, see
-- re2_vr_mod_v1_4_0_release memory). Known/suspected game-breaking; kept in
-- the mod as an opt-in checkbox for anyone who wants to help test it, not as
-- a trusted default. The merge bug (world pickup object not despawning after
...
local state = {
    ...
    -- Enabled by default 2026-08-14 (v1.4.0) per player's request -- shows
    -- the clean "new item" card for every pickup instead of the full grid.
    -- Confirmed working since 2026-08-10, this is a display-only toggle,
    -- doesn't affect whether the item is actually granted.
    force_new_item_look = true,
