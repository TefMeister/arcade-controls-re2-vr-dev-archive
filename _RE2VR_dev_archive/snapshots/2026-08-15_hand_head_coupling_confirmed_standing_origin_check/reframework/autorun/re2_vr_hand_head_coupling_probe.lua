...
local function trace_fp_hand_pos()
    ...
    -- 2026-08-15, confirmed live: hand_pos DOES swing with head yaw alone
    -- (controller held still, ~0.012 units of hand_pos movement per degree
    -- of yaw change across 6192 sampled frame-pairs). ctrl_pos/hmd_pos are
    -- small-magnitude numbers (e.g. ~0.25, -0.29, -0.47) -- clearly a local
    -- tracking-space frame, NOT already world-space (real world coords here
    -- are ~-12, 1.5, -18) -- so SOME rotation is genuinely needed to convert
    -- the offset into world space. Suspect: it should be a FIXED play-space
    -- calibration yaw, not the LIVE camera yaw re-read every frame.
    -- re2_vr_ladder_camera_status.md already theorizes
    -- vrmod:get_standing_origin()'s `w` component is exactly this kind of
    -- fixed calibration reference (unconfirmed there too). Logging it here,
    -- read-only, to see whether IT stays constant while cam_yaw changes --
    -- if so, it's the right thing to substitute in place of live yaw.
    local standing_origin = safe(function() return vrmod:get_standing_origin() end)

    state.frame = state.frame + 1
    log.info(string.format(
        "[hand_head_probe] frame=%d cam_yaw=%.2f  standing_origin=%s  hmd_pos=%s  ctrl_pos=%s  offset=%s (len=%.4f)  hand_pos=%s",
        state.frame,
        yaw_deg(cam_rot) or -999,
        standing_origin and string.format("(%.4f, %.4f, %.4f, w=%s)",
            standing_origin.x, standing_origin.y, standing_origin.z, tostring(standing_origin.w)) or "nil",
