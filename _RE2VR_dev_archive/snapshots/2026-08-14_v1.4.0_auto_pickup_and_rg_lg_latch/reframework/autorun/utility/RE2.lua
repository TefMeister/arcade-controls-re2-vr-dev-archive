...
function RE2.get_localplayer()
    ...
    local ok, player = pcall(function() return playman:call("get_CurrentPlayer") end)
...
function RE2.get_weapon_object(player)
    ...
    local ok, weapon_gameobject = pcall(function() return weapon:call("get_GameObject") end)
    if not ok or not weapon_gameobject then
        return nil, nil
    end
