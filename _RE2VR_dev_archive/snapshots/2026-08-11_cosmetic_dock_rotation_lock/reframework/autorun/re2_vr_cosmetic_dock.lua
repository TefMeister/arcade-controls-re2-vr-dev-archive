...
local function get_grip_rot_pitch_for_weapon(wp_id)
    if wp_id and type(GRIP_ROT_PITCH_BY_WP[wp_id]) == "number" then
        return GRIP_ROT_PITCH_BY_WP[wp_id]
    end
    return clampf(tonumber(CFG.grip_rot_pitch_deg) or 0.0, -180.0, 180.0)
end

local function get_grip_rot_yaw_for_weapon(wp_id)
    if wp_id and type(GRIP_ROT_YAW_BY_WP[wp_id]) == "number" then
        return GRIP_ROT_YAW_BY_WP[wp_id]
    end
    return clampf(tonumber(CFG.grip_rot_yaw_deg) or 0.0, -180.0, 180.0)
end

local function get_grip_rot_roll_for_weapon(wp_id)
    if wp_id and type(GRIP_ROT_ROLL_BY_WP[wp_id]) == "number" then
        return GRIP_ROT_ROLL_BY_WP[wp_id]
    end
    return clampf(tonumber(CFG.grip_rot_roll_deg) or 0.0, -180.0, 180.0)
end
...
local function get_weapon_support_grip_world_rot(wp_id, muzzle_rot)
    if not muzzle_rot then return nil end
    local pitch = get_grip_rot_pitch_for_weapon(wp_id)
    local yaw = get_grip_rot_yaw_for_weapon(wp_id)
    local roll = get_grip_rot_roll_for_weapon(wp_id)
...
--- so the hand's orientation freezes too, not just its position.
local function get_weapon_support_grip_world_pos(wp_id)
    ...
    return anchor, get_weapon_support_grip_world_rot(wp_id, muzzle_rot)
end
...
re.on_frame(function()
    ...
    local anchor, anchor_rot = get_weapon_support_grip_world_pos(wp_id)
...
re.on_draw_ui(function()
    ...
    if wp_id and COSMETIC_DOCK_WEAPONS[wp_id] then
        ...
        local cur_p = get_grip_rot_pitch_for_weapon(wp_id)
        local changed_p, new_p = imgui.slider_float(
            "Grip pitch (deg) [" .. wp_id .. "]", cur_p, -180.0, 180.0, "%.1f")
        if changed_p then
            GRIP_ROT_PITCH_BY_WP[wp_id] = new_p
            save_cfg()
        end

        local cur_y = get_grip_rot_yaw_for_weapon(wp_id)
        local changed_y, new_y = imgui.slider_float(
            "Grip yaw (deg) [" .. wp_id .. "]", cur_y, -180.0, 180.0, "%.1f")
        if changed_y then
            GRIP_ROT_YAW_BY_WP[wp_id] = new_y
            save_cfg()
        end

        local cur_ro = get_grip_rot_roll_for_weapon(wp_id)
        local changed_ro, new_ro = imgui.slider_float(
            "Grip roll (deg) [" .. wp_id .. "]", cur_ro, -180.0, 180.0, "%.1f")
        if changed_ro then
            GRIP_ROT_ROLL_BY_WP[wp_id] = new_ro
            save_cfg()
        end
    else
