...
local state = {
    ...
    -- The quaternion multiply order (parent-side pre-multiply) was
    -- confirmed correct live 2026-08-16; the A/B flip switch was removed
    -- in the 2026-08-17 release cleanup.
    soft_mode = true,
    ...
    -- 2026-08-17 release cleanup (player request): manual aim compensation
    -- (the wrist-nudge sliders) REMOVED -- it was the workaround for the
    -- red-dot drift, obsolete since the 6x/frame IK root fix. Also removed:
    -- hips/legs/root-chain diagnostic freezes (all ruled out during the
    -- residual-sway hunt), the Post/Prepare hook-timing options (both
    -- tested and rejected -- Pre is hardcoded now), the soft-mode
    -- multiply-order A/B flip (default order confirmed correct live), and
    -- the joint-name dump button. All recoverable from the dev archive.
}
        ...
        if joint then
            ...
            if ok_r and r and type(r.y) == "number" then
                ...
                if state.soft_mode then
                    ...
                    local q_out = quat_mul(q_off, q_anim)
...
-- Hook timing is hardcoded to PRE (before arm IK solves). Post and Prepare
-- were both live-tested and rejected (2026-08-05/15): they write after arm
-- IK already solved against the uncorrected spine (gun points left) -- the
-- selectable timing UI was removed in the 2026-08-17 release cleanup.
