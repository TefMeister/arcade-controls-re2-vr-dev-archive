...
function M.init(deps)
    ...
    local function get_controller_raw_openxr_pos()
        ...
        if vrc_manager then
            ...
            if ok_has and has then
                ...
                if list and list[1] and type(list[1].position) == "table" and type(list[1].position.x) == "number" then
        ...
        if get_vr_controller_world_pos then
            local cp = get_vr_controller_world_pos("left")
    ...
    local function get_controller_game_world_pos()
        local raw, raw_src = get_controller_raw_openxr_pos()
    ...
    local function try_start_rack_gesture(wp, track_pos)
        ...
        if not is_track_near_dock(wp, track_pos) then return false end
    ...
    local function update_slide_rack_trigger(wp, publish_visual)
        ...
        if not grip then
            -- Grip let go before the cycle finished: abort back to parked,
            -- same as letting go of the pump handle early does.
            rack.active = false
            rack.armed = true
            rack.trig_travel = 0.0
            rack.trig_committed = false
        ...
        if not rack.pull_done and rack.trig_committed and rack.trig_travel >= 1.0 - 1e-4 then
    ...
    function M.tick()
        ...
        local grip = is_left_grip_pressed()
        local grip_prev = rack.last_grip == true
        ...
        if rack.needs_rack and dock_pos then
            ...
            near = rack.last_dock_dist <= get_dock_dist(wp)
        end
        rack.in_range = near
        ...
        if rack.needs_rack and grip and not grip_prev and not is_mag_insert_active()
            and get_bullet_number() > 0 and near then
            try_start_rack_gesture(wp, track_pos)
        end
        ...
        tick_hand_dock_blend(wp, grip, rack.active)
