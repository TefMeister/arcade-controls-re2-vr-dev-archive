...
local function update_manual_pump_gesture()
    ...
    if gesture.needs_pump and grip then
        ...
        gesture.last_track_src = track_src
        if signed ~= nil then
            if not gesture.pull_dir_locked and math.abs(signed) > dz then
                gesture.pull_increases_signed = signed > (gesture.pull_start_signed or 0.0)
                gesture.pull_dir_locked = true
            end
            local pull_reached = false
            if gesture.pull_increases_signed then
                pull_reached = signed >= pull_d - 1e-5
            else
                pull_reached = signed <= -pull_d + 1e-5
            end
            if not gesture.pull_done and pull_d > 1e-5 and pull_reached then
                gesture.pull_done = true
                gesture.pull_peak_signed = signed
                if slide_gesture.gesture_capture_pull_peak then
                    slide_gesture.gesture_capture_pull_peak(gesture)
                end
            ...
            if gesture.pull_done and push_d > 1e-5 and get_push_delta then
                local push_delta = get_push_delta(gesture, signed)
    ...
    elseif gesture.active and not grip then
        if gesture.pull_done and get_push_delta then
            local signed = gesture.pull_now or 0.0
            local push_delta = get_push_delta(gesture, signed)
            if push_d > 1e-5 and push_delta >= push_d * 0.55 then
                complete_pump_cycle(wp)
            else
                gesture.active = false
                clear_gesture_pull_state()
