    ...
    if gesture.needs_pump and grip then
        if not gesture.active then
            if not gesture.pump_await_grip_release then
                ...
                -- the hand-follow offset and bind-pose blend still work.
            end
        end
        ...
        if gesture.active then
            ...
            -- (e.g. the hand-follow offset), so use the weapon's configured
            -- pull distance rather than a raw 0/1 flag.
