...
local MAG_HOLSTER_DEF = { off_right = 0.0, off_up = 0.0, off_forward = 0.0, trigger_dist = 0.25, release_dist = 0.38 }
...
local function seed_mag_holster_profile_entry(key)
    ...
    if type(parent) == "table" then
        return {
            off_right = tonumber(parent.off_right) or MAG_HOLSTER_DEF.off_right,
            off_up = tonumber(parent.off_up) or MAG_HOLSTER_DEF.off_up,
            off_forward = tonumber(parent.off_forward) or MAG_HOLSTER_DEF.off_forward,
        }
    end
    return {
        off_right = tonumber(deps_ref.CFG.mag_holster and deps_ref.CFG.mag_holster.off_right) or MAG_HOLSTER_DEF.off_right,
        off_up = tonumber(deps_ref.CFG.mag_holster and deps_ref.CFG.mag_holster.off_up) or MAG_HOLSTER_DEF.off_up,
        off_forward = tonumber(deps_ref.CFG.mag_holster and deps_ref.CFG.mag_holster.off_forward) or MAG_HOLSTER_DEF.off_forward,
    }
end
...
local function read_left_grip_action_active(lj)
    ...
    if not mag_hand.grip_action_cache then
...
local function get_left_hand_position()
    local cached = rawget(_G, "__vr_lh_joint_pos")
...
local function handle_mag_reload_b_edge()
    ...
    M.get_vr_controller_world_pos = function() return get_vr_controller_world_pos("left") end
