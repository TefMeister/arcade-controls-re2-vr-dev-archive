...
--              trigger.
--   Attempt 6: found setForceEquipType(Nullable<WeaponType>,
--              Nullable<WeaponParts>, bool) -- a real method distinct from
--              the property -> threw the SAME "Invoke threw an exception"
--              error every frame, confirming this isn't specific to one
--              method: ANY method call() with a Nullable<WeaponType> param
--              throws, full stop.
--   Attempt 7 (this one): only ONE thing has ever run clean without
--              throwing -- the raw field write on ForceEquipType (attempt
--              3), which also got a real reaction (the equip sound) but
--              reverted with no visual change. So: keep the raw field write
--              (known safe, no marshaling issue), and follow it with an
--              explicit requestEquip() call -- a method with NO parameters,
--              so it can't hit the same Nullable wall -- to see if that's
--              the missing "now apply it" step attempt 3 needed.
...
local function start_force(equipment)
    ...
    if not ok2 then
        log.warn("[re2_vr_subweapon_trigger] Writing ForceEquipType failed: " .. tostring(err2))
        return
    end

    local ok3, err3 = pcall(function()
        equipment:call("requestEquip")
    end)
    if ok3 then
        state.forcing = true
    else
        log.warn("[re2_vr_subweapon_trigger] requestEquip(sub) failed: " .. tostring(err3))
    end
end
...
local function stop_force(equipment)
    ...
    if not ok1 then
        log.warn("[re2_vr_subweapon_trigger] Clearing ForceEquipType failed: " .. tostring(err1))
    end
    ...
    if not ok2 then
        log.warn("[re2_vr_subweapon_trigger] requestEquip(clear) failed: " .. tostring(err2))
    end
